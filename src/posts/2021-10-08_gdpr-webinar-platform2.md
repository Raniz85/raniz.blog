...
either American or use American service providers which disqualifies them according to our rules.

In this post, we're going to explore open source alternatives, both hosted and running them ourselves.

---

First though, let's review those rules:
...
at the time of writing - contains five commercial solutions. Of these, the only company operating in a country on
_the list_ (read the previous post) is [Blindside Networks](https://blindsidenetworks.com) (the company developing BBB)
from Canada. Unfortunately they use AWS so we can't use them either.
...
## Self-Hosting
I've been running services in production for more than 15 years so I'm not afraid to run my own software when needed.
...
server in our office, [if we had one](https://www.linkedin.com/pulse/kontanter-bensin-kontor-jimmy-nilsson/).
...
certificates on Ubuntu and get it working with a self-signed certificate, though I gave up and installed it on a real
server instead (well, actually a VM at [CityCloud](https://citycloud.se)).

## Evaluation
So, with BBB up and running we tested it out. Initially it seems to work great. You can limit sign-ups to invite-only
and give users with emails belonging to a specific domain moderator privileges by default (this is all in Greenlight by
the way). Meetings can be joined anonymously and there is recording support out of the box.

When it comes to recordings it all unfortunately fell apart. BBB "records" the meeting and then replays all video
and audio streams individually on the player page - there is no easy way of getting a video file out of it that you can
share online.
...
supports up to 100 participants per meeting. You can record meetings and upload them to DropBox or live stream them
over RTMP to a service of your choice (such as Twitch or YouTube Live).
...
It works better than Google Meet on Firefox, has background blurring/replacement and even has push to talk if you're
muted. The only issue we've had with Jitsi so far is the tiles jumping around in the tiled layout from time to time.

Heck, you can even [add it to Google Calendar](
https://workspace.google.com/marketplace/app/jitsi_meet_for_google_calendar/421935948107) - that extension is [open
source](https://github.com/jitsi/calendar-addon) by the way so if you manage to figure out how to publish an internal
Google Calendar extension you could use it with your self-hosted instance.

### The Big Issue
The big issue with Jitsi Meet is the number of participants. At the time of writing, Jitsi Meet recommends no more than
35 participants on a single server (though supports more). This clearly isn't enough for us so would disqualify Jitsi.
...
work for me at this point though. I'm sure I could figure it out eventually but this is starting to get too complex.
...
We're keeping the Jitsi Meet instance around and once I get the hang of Google Apps Script we'll probably switch our
calendar bookings over to it as well. I'm not sure if we have decided on where to host the [next webinar](
...
(I'm amazed and thrilled anyone even bothered to read it) and among those were a few tips about other webinar platforms
that we could have looked at. I have not looked thoroughly at all of these but here's a short list if you want to dig
a bit deeper yourself. All of these are likely better candidates than the first 10 in the previous post.

### Eyeson
A German webinar/meeting platform that has an interesting approach: instead of broadcasting _everyone's_ camera feed to
_everyone_ else in the meeting they encode _one_ video stream on their backend and broadcast that to everyone. It's
essentially the setup we did with Jitsi + Solidtango but with everyones webcams and completely integrated. I think it's
an interesting approach but unfortunately everyone in the meeting can stop the ongoing recording which makes hosting a
webinar there that you intend to make available afterwards risky.
...
because we had already decided that Jitsi + streaming was the way forward.
...
### Quickchannel
[Quickchannel](https://www.quickchannel.com) is a Swedish streaming, video and webinar platform that - according to
their CEO who emailed us after the webinar - fulfill all of our requirements. I almost didn't put them up here because
I didn't find them myself and including them after a tip from their CEO almost felt like advertising.

However, he assures me that they're running their own servers and their site says that you can run their software
completely on-prem so they do actually seem to be an alternative worth mentioning. I haven't tested it because I
couldn't find any free offering or test account - and we're not looking so I haven't bothered to contact them about it.